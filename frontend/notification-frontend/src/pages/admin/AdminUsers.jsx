...
export default function AdminUsers() {
  ...
  async function fetchPage({ nextPage, nextLimit, nextFilters } = {}) {
    ...
    try {
      ...
      // In case API clamps page, keep UI in sync.
      if (data?.meta?.page && data.meta.page !== pageToUse) {
        setPage(data.meta.page);
      }
      if (data?.meta?.limit && data.meta.limit !== limitToUse) {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        key: "role",
        title: "Role",
        render: (u) => <Badge color={u.role === "admin" ? "blue" : u.role === "creator" ? "yellow" : u.role === "viewer" ? "slate" : "green"}>{u.role}</Badge>,
      },
      {
        key: "city",
        title: "City",
  ...
  return (
    <div>
      <PageHeader
        title="Users"
        subtitle="Create users, update city/status, upload CSV, and delete users."
        right={
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card title="Create User">
          <form className="space-y-3" onSubmit={onCreate}>
            ...
            <FormField label="Password" hint="Stored hashed in DB">
      ...
      <UserDetailsModal
        ...
        onDelete={async (u) => {
          try {
            ...
            // If last row on page deleted, go back a page.
