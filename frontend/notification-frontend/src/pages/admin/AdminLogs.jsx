...
export default function AdminLogs() {
  ...
  const typeOptions = useMemo(() => {
    // Show fixed options since we now have data that might not be in the current list
    ...
    () => [
      {
        key: "sent_at",
        title: "Date",
        render: (l) => <span className="text-slate-700 dark:text-slate-200">{formatDateTime(l.sent_at)}</span>,
      },
      {
        key: "user",
        title: "User",
      ...
      },
      {
        key: "status",
        title: "Status",
        render: (l) => <Badge color={l.status === "success" ? "green" : "red"}>{l.status}</Badge>,
      },
    ],
    []
  );
  ...
  return (
    <div>
      <PageHeader title="Logs" subtitle="Delivery logs for sent campaigns." />
